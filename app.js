...
const universidades = [
  { value: 'UMAYOR', name: 'Universidad Mayor' }
];
...
function renderUniversidades() {
  const usel = document.getElementById('universidad-select');
  ...
  usel.addEventListener('change', onUniChange);
}

function onUniChange(e) {
  const uni = e.target.value;
  const csel = document.getElementById('carrera-select');
  csel.innerHTML = '<option value="" disabled selected>Elige tu carrera…</option>';
  uniCarreras[uni].forEach(code => {
    const o = document.createElement('option');
    o.value = code;
    o.textContent = carreraNombres[code] || carrerasData[code][0].nombre;
    csel.appendChild(o);
  });
  csel.disabled = false;
  csel.addEventListener('change', onCarreraChange);
}

function onCarreraChange(e) {
  document.querySelector('.grid-header').style.display = 'grid';
...
function renderMalla(asigs) {
  ...
  asigs.forEach(a => {
    ...
    if (a.codigo === '40' || a.codigo === '46') {
    div.classList.add('texto-grande');
  }

  if (a.codigo === '43') {
    div.classList.add('texto-43');
  }

  if (a.codigo === '10') {
    div.classList.add('texto-10');
  }

  if (a.codigo === '29') {
    div.classList.add('texto-29');
  }

  if (a.codigo === '54') {
    div.classList.add('texto-54');
  }

  if (a.codigo === '60') {
    div.classList.add('texto-60');
  }
  ...
  document.querySelectorAll('.grid-item').forEach(el => {
    ...
    if (codigo === '56') {
      ok = Array.from(document.querySelectorAll('.grid-item'))
        .filter(item => {
          const a = carrerasData['MEDVET'].find(x => x.codigo === item.dataset.codigo);
